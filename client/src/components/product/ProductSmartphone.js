...
function ProductSmartphone() {
    ...
    return (
        <div>
            <CardGroup>
                <Col><h2 style={{ textAlign: "center", margin: "50px" }}>Produit Smartphone </h2></Col>
                <Container fluid >
                    <Row >
                        <Col sm style={{ display: 'flex', flexWrap: "wrap", justifyContent: "center" }}>
                            {!product ? <Spinner animation="border" /> : product.filter((el) => el.category == "SMARTPHONE").map((e) =>
...
export default ProductSmartphone
